 ...
 * @Last Modified time: 2020-05-03 14:46:14
...
class Track_visitor {
    ...
    private $CONTROLLER_IGNORE_LIST = array(
        'panel',"users","pliki","acl","gallery","menu","admin/news","pages","slider","szablon","pages_p",'admin'
    );
    ...
    private function log_visitor() {
        if ($this->track_session() === TRUE) {
            ...
            $temp_visitor_id = $this->session->userdata('visitor_id');
            $visitor_id = isset($temp_visitor_id) ? $temp_visitor_id : 0;
            $no_of_visits = $this->session->userdata('no_of_visits');
            $current_page = current_url();
            $temp_current_page = $this->session->userdata('current_page');
            if (isset($temp_current_page) && $temp_current_page != $current_page) {
                $page_name = $this->router->fetch_class() . '/' . $this->router->fetch_method();
                $page_length = strlen(trim($this->router->fetch_class() . '/' . $this->router->fetch_method()));
                $query_params = trim(substr($this->uri->uri_string(), $page_length + 1));
                $query_string = strlen($query_params) ? $query_params : '';
                $data = array(
                    'no_of_visits' => $no_of_visits,
                    'ip_address' => $this->input->server('REMOTE_ADDR'),
                    'requested_url' => $this->input->server('REQUEST_URI'),
                    'referer_page' => $this->agent->referrer(),
                    'user_agent' => $this->agent->agent_string(),
                    'page_name' => $page_name,
                    'query_string' => $query_string
                );
                $this->db->insert($this->site_log, $data);
            ...
            $result = $this->db->insert($this->site_log, $data);
            if ($result === FALSE) {
                /**
                 * find the next available visitor_id for the database
                 * to assign to this person
                 */
                $this->session->set_userdata('track_session', FALSE);
            } else {
                /**
                 * find the next available visitor_id for the database
                 * to assign to this person
                 */
                $this->session->set_userdata('track_session', TRUE);
                $entry_id = $this->db->insert_id();
                $query = $this->db->query('select max(no_of_visits) as next from ' .
                ...
                $this->db->where('site_log_id', $entry_id);
                $data = array(
                    'no_of_visits' => $count
                );
                $this->db->update($this->site_log, $data);
                //place the current visitor_id into the session so we can use it on
                //subsequent visits to track this person
                $this->session->set_userdata('no_of_visits', $count);
